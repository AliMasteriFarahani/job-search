class CustomSelect {
  ...
  createElements() {
    ...
    customSelects.forEach((element) => {
      ...
      // element.appendChild(customSelectContainer)
  ...
  selectOption(e) {
    if (
      ...
      let originalSelectedOption = e.target
        ...
        .nextElementSibling.querySelector(`option[selected='selected']`);
      customOptionSelected.innerText = customOptionText;
      originalSelectedOption
        ? originalSelectedOption.removeAttribute("selected")
        : "";
      originalOptionSelect.setAttribute("selected", "selected");

      // when click an option it's must be close
  ...
  onInput(text, e) {
   // let inpSearch = e.target.parentElement.querySelector(".search-inp");
    text === ""
      ? (this.optionFiltered = this.optionsObj)
